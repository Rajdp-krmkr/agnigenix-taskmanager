...
import { FaClock, FaStopwatch, FaChartLine, FaTrophy, FaCalendarCheck } from 'react-icons/fa'
...
const AverageCompletionTime = ({ projectTasks = [], isLoading = false }) => {
    const completionData = useMemo(() => {
        ...
        const priorityAverages = Object.entries(priorityStats).map(([priority, times]) => {
        ...
        return {
            ...
            performanceCategory,
            avgDays: avgHours / 24,
            medianTime: completedTaskTimes.length > 0 ? 
                completedTaskTimes[Math.floor(completedTaskTimes.length / 2)]?.completionTime : 0
        };
    }, [projectTasks]);
    ...
    const getPriorityIcon = (priority) => {
        switch (priority) {
            case 'high': return { icon: FaTrophy, color: 'text-red-500', bg: 'bg-red-100 dark:bg-red-900/20' };
            case 'medium': return { icon: FaStopwatch, color: 'text-yellow-500', bg: 'bg-yellow-100 dark:bg-yellow-900/20' };
            case 'low': return { icon: FaCalendarCheck, color: 'text-green-500', bg: 'bg-green-100 dark:bg-green-900/20' };
            default: return { icon: FaClock, color: 'text-gray-500', bg: 'bg-gray-100 dark:bg-gray-900/20' };
        }
    };
    ...
    const getPerformanceText = (category) => {
        switch (category) {
            ...
            case 'needs_improvement': return '🎯 Room for Improvement';
    ...
    const { avgHours, totalCompletedTasks, priorityAverages, fastestTask, slowestTask, 
            performanceCategory, avgDays, medianTime } = completionData;
    ...
    return (
        <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-sm">
            ...
            <div className="flex items-center justify-between mb-6">
                ...
                <div className={`px-3 py-1 rounded-full ${getPerformanceColor(performanceCategory)}`}>
                ...
                <div className="mb-6">
                    ...
                    <div className="space-y-3">
                        {priorityAverages.map((item, index) => {
                            ...
                            return (
                                <div key={index} className={`flex items-center justify-between p-3 ${priorityInfo.bg} rounded-lg`}>
                                    <div className="flex items-center gap-3">
            ...
            {/* Insights */}
            <div className={`p-4 rounded-lg ${getPerformanceColor(performanceCategory)}`}>
                ...
                <div className="text-sm">
                    ...
                    {avgHours >= 72 && avgHours < 168 && (
                        <p>⚡ Tasks typically take 3-7 days to complete. Consider breaking down complex tasks.</p>
                    )}
                    {avgHours >= 168 && (
                        <p>🎯 Tasks take over a week on average. Consider reviewing task complexity and resource allocation.</p>
                    )}
                    
                    {slowestTask && (
                        <p className="mt-2 text-xs">
                            💡 Longest task: &ldquo;{slowestTask.title}&rdquo; took {formatTime(slowestTask.completionTime)}
                        </p>
                    )}
                </div>
            </div>
